...
return new class extends Migration
{
    ...
    {
        DB::statement('DROP VIEW IF EXISTS `' . env('DB_DATABASE') . '`.`qtydropship`;');
    ...
    VIEW `' . env('DB_DATABASE') . '`.`qtydropship` AS
    SELECT 
        ...
        COALESCE(SUM((CASE
                ...
                0) AS `QTY`
    FROM
        ((((`' . env('DB_DATABASE') . '`.`soitem`
        JOIN `' . env('DB_DATABASE') . '`.`product` ON ((`' . env('DB_DATABASE') . '`.`product`.`id` = `' . env('DB_DATABASE') . '`.`soitem`.`productId`)))
        JOIN `' . env('DB_DATABASE') . '`.`part` ON ((`' . env('DB_DATABASE') . '`.`part`.`id` = `' . env('DB_DATABASE') . '`.`product`.`partId`)))
    ...
    WHERE
        ((`' . env('DB_DATABASE') . '`.`so`.`statusId` IN (20, 25))
            AND (`' . env('DB_DATABASE') . '`.`soitem`.`statusId` IN (10, 14, 20, 30, 40))
            AND (`' . env('DB_DATABASE') . '`.`soitem`.`typeId` = 12))
