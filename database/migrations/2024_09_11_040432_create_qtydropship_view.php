...
VIEW `lilypad`.`qtydropship` AS
    ...
    FROM
        ((((`lilypad`.`soitem`
        JOIN `lilypad`.`product` ON ((`lilypad`.`product`.`id` = `lilypad`.`soitem`.`productId`)))
        JOIN `lilypad`.`part` ON ((`lilypad`.`part`.`id` = `lilypad`.`product`.`partId`)))
    ...
    {
        Schema::dropIfExists('qtydropship_view');
    }
};
