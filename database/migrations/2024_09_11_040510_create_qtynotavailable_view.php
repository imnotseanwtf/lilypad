...
return new class extends Migration
{
    ...
    {
        DB::statement('DROP VIEW IF EXISTS `' . env('DB_DATABASE') . '`.`qtynotavailable`;');
        ...
        VIEW `' . env('DB_DATABASE') . '`.`qtynotavailable` AS
        SELECT 
        ...
        WHERE
            ((`' . env('DB_DATABASE') . '`.`tag`.`typeId` IN (30, 40))
                AND (`' . env('DB_DATABASE') . '`.`location`.`countedAsAvailable` = 0)
